#!/bin/bash
# run_hive_job32.sh — Esegue il Job 3.2 in Hive per un subset specifico o per tutti
#
# Uso:
#   bash run_hive_job32.sh [subset] [base_path]
#
# [subset] può essere:
...
LOGDIR="./logs/hive_job32_$(date +%Y%m%d_%H%M%S)"
mkdir -p ${LOGDIR}
...
run_hive_subset() {
    ...
    echo ""
    echo "═══════════════════════════════════════════════"
    echo "  Eseguendo: Job 3.2 (Hive HQL)  [subset=${SUB}]"
    ...
    docker exec -i hiveserver2 beeline \
        ...
        < jobs/02_job32_hive.hql 2>&1 | tee "${LOGDIR}/Job32_hive_${SUB}.log"
    ...
    echo "job32,hive,${SUB},local,${ELAPSED}" >> "${TIMING_FILE}"
    ...
    if [ "${SUBSET}" != "all" ] && [ -f "./results/timings_local_all.txt" ]; then
      echo "job32,hive,${SUB},local,${ELAPSED}" >> "./results/timings_local_all.txt"
    fi

    echo "  ✅ Job 3.2 Hive (${SUB}) completato in: ${ELAPSED}s"
}
