#!/bin/bash
# run_hive_job31.sh — Esegue il Job 3.1 in Hive per un subset specifico o per tutti
#
# Uso:
#   bash run_hive_job31.sh [subset]
#
# [subset] può essere:
...
LOGDIR="./logs/hive_$(date +%Y%m%d_%H%M%S)"
mkdir -p ${LOGDIR}
...
run_hive_subset() {
    ...
    echo ""
    echo "═══════════════════════════════════════════════"
    echo "  Eseguendo: Job 3.1 (Hive HQL)  [subset=${SUB}]"
    ...
    docker exec -i hiveserver2 beeline \
        ...
        < jobs/01_job31_hive.hql 2>&1 | tee "${LOGDIR}/Job31_hive_${SUB}.log"
    ...
    echo "job31,hive,${SUB},local,${ELAPSED}" >> "${TIMING_FILE}"
    ...
    if [ "${SUBSET}" != "all" ] && [ -f "./results/timings_local_all.txt" ]; then
      echo "job31,hive,${SUB},local,${ELAPSED}" >> "./results/timings_local_all.txt"
    fi
    
    echo "  ✅ Job 3.1 Hive (${SUB}) completato in: ${ELAPSED}s"
}
